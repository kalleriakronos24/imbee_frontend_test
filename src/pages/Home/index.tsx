...
const Home = () => {
  const { isFetchEnd, questionData, setQuestionSteps, getQuestions } = useRetrieveQuestion();
              ...
              <div className="px-8 py-16">
                ...
                <div className="flex flex-row w-full mt-6">
                  {topicData.length > 0 && topicData.map((topic) => {
                    return (
                      <span onClick={() => setSelectedTopicParam(topic.name)} key={topic.name} className={`cursor-pointer mr-4 px-6 py-2 rounded-full ${topic.isSelected && "border-blue-300"} border-2`}>{topic.name}</span>
                    )
                  })}
                  ...
                  >
                    <div className="py-8 flex flex-col px-6">
                      {questionData.map(question => {
                        return (
                          <div key={question.title} onClick={() => window.location.href = question.link} className="flex flex-row mb-6 justify-between items-center w-full">
                            <div className="w-[50%] flex flex-col cursor-pointer">
                              <span className="font-medium text-lg">{question.title.length >= 63 ? question.title.slice(0, 63) + "..." : question.title}</span>
                              <div className="flex flex-row my-4 justify-between">
                                <div className="flex flex-col">
                                  <span className="text-green-500">Score</span>
                                  <span className={`text-center ${question.score < 0 ? "text-red-500" : "text-black"}`}>{question.score}</span>
                                </div>
                                <div className="flex flex-col">
                                  <span className="text-green-500">Answers</span>
                                  <span className={`text-center  ${question.is_answered ? "bg-green-500 text-white" : question.answer_count > 0 ? "text-green-500 border-2 border-green-500" : "text-black"}`}>{question.answer_count}</span>
                                </div>
                                <div className="flex flex-col">
                                  <span className="text-green-500">Viewed</span>
                                  <span className="text-center">{question.view_count}</span>
                                </div>
                              </div>
                            </div>
                            <div onClick={() => window.location.href = question.owner.link} className="flex flex-col items-center mt-4 cursor-pointer">
                              <img id={`imgid-${question.title}`} src={question.owner.profile_image} className="h-[70px] w-[70px] rounded-full" alt="img" />
                              <span className="text-center mt-4">{question.owner.display_name}</span>
                            </div>
                            <hr className="mt-2" />
                          </div>
                        );
